...
class StableDiffusionPipeline(diffusers.DiffusionPipeline):
    def __init__(
        ...
        if hasattr(scheduler.config, "steps_offset") and scheduler.config.steps_offset != 1:
            ...
            scheduler._internal_dict = diffusers.FrozenDict(new_config)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if isinstance(self.scheduler, diffusers.LMSDiscreteScheduler):
...
class StableDiffusionMaskingPipeline(diffusers.DiffusionPipeline):
    ...
    def __call__(
        ...
        latents = init_latents
        t_start = max(num_inference_steps - init_timestep + offset, 0)
        for i, t in tqdm.auto.tqdm(enumerate(self.scheduler.timesteps[t_start:])):
            t_index = t_start + i
            # expand the latents if we are doing classifier free guidance
            latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
            if isinstance(self.scheduler, diffusers.LMSDiscreteScheduler):
